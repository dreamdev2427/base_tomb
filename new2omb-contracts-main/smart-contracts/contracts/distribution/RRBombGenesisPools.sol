...
// Note that this pool has no minter key of RRBOMB (rewards).
// Instead, the governance will call RRBOMB distributeReward method and send reward to this pool at the beginning.
contract RRBombGenesisPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. RRBOMB to distribute.
        uint256 lastRewardTime; // Last time that RRBOMB distribution occurs.
        uint256 accTombPerShare; // Accumulated RRBOMB per share, times 1e18. See below.
    ...
    function safeTombTransfer(address _to, uint256 _amount) internal {
        ...
        if (_tombBalance > 0) {
            if (_amount > _tombBalance) {
                tomb.transfer(_to, _tombBalance);
            } else {
