...
//     "0x6398ACBBAB2561553a9e458Ab67dCFbD58944e52", // 2shares/ETH LP
//     "0x83A52eff2E9D112E9B022399A9fD22a9DB7d33Ae", // BaseTomb/WETH
//     "0x6437ADAC543583C4b31Bf0323A0870430F5CC2e7", // 3shares
//     "0xd352daC95a91AfeFb112DBBB3463ccfA5EC15b65", // 3shares/WETH
// ]
...
function useTotalTreasuryBalance() {
  ...
  const [balance_2shares_weth, setBalance_2shares_weth] = useState(0);
  const [balance_3omb_weth, setBalance_3omb_weth] = useState(0);
  const [balance_3shares_weth, setBalance_3shares_weth] = useState(0);
  ...
  async function getBalance() {
    ...
    const data2sharesAnd3omb = await axios(
      'https://openapi.debank.com/v1/user/chain_balance?id=0x8f555E00ea0FAc871b3Aa70C015915dB094E7f88&chain_id=eth',
    );

    console.log(`3Shares USD: $${value3shares}`);
    console.log(`2Shares + BOMB: $${data2sharesAnd3omb.data.usd_value}`);
    const LP_2shares_weth = await getLPPrice(
      '0x6398ACBBAB2561553a9e458Ab67dCFbD58944e52',
      '0xc54a1684fd1bef1f077a336e6be4bd9a3096a6ca',
    );
    const LP_3omb_weth = await getLPPrice(
      '0x83A52eff2E9D112E9B022399A9fD22a9DB7d33Ae',
      '0x14def7584a6c52f470ca4f4b9671056b22f4ffde',
    );
    const LP_3shares_weth = await getLPPrice(
      '0xd352daC95a91AfeFb112DBBB3463ccfA5EC15b65',
      '0x6437adac543583c4b31bf0323a0870430f5cc2e7',
    );
    setBalance(data2sharesAnd3omb.data.usd_value + value3shares + LP_2shares_weth + LP_3omb_weth + LP_3shares_weth);
    setBalance_2shares_weth(LP_2shares_weth);
    setBalance_3omb_weth(LP_3omb_weth);
    setBalance_3shares_weth(LP_3shares_weth);
  ...
  async function get3ombBalance() {
    ...
    const usdValue =
      Number(web3.utils.fromWei(await token3omb.methods.balanceOf(treasuryAddress).call())) * Number(data.price);
  ...
  async function get3sharesBalance() {
    ...
    const usdValue =
      Number(web3.utils.fromWei(await token3shares.methods.balanceOf(treasuryAddress).call())) * Number(data.price);
  ...
  async function getLPPrice(LPAddress, tokenAddress) {
    ...
    const { data } = await axios('https://api.binance.com/api/v1/ticker/price?symbol=ETHUSDT');
    const wethValue = Number(web3.utils.fromWei(await WETH.methods.balanceOf(LPAddress).call())) * Number(data.price);
    ...
    const OneTokenValue =
      (wethValue + tokenValue) / Number(web3.utils.fromWei(await LPtoken.methods.totalSupply().call()));
